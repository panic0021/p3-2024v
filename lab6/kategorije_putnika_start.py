from putnik_start import Putnik
from flight_enum_start import UslugaNaLetu
from sys import stderr
class PutnikEkonomskeKlase(Putnik):
    def dodaj_izabrane_usluge(self,recnik):
        ...
        for kljuc,vrednost in recnik.items():
            if type(kljuc) is UslugaNaLetu:
                dodatne_usluge.append(kljuc)
                uvecanje_cene+=vrednost
        
        self.cena_karte+=uvecanje_cene
        self.usluge.extend(dodatne_usluge) #extend dodaje elemente pojedinacno, raspakuje prvo
        #self.usluge.append(dodatne_usluge) bi dodao kao jedan element celu listu objekata
    ...
    def __str__(self):
        putnik_str=super().__str__()#pozivanje nadklase
    ...
    def __init__(self,usluge_na_letu=(UslugaNaLetu.PRIORITETNO_UKRCAVANJE,),**kwargs):
        ...
        for usluga in usluge_na_letu:
            if isinstance(usluga,UslugaNaLetu):
                ...
                stderr.write(f"Greska! Neadekvatna vrednost {usluga}")

    def __str__(self):
        putnik_str=super().__str__()
        return putnik_str.replace('Putnik','Putnik biznis klase')
